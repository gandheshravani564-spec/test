...
# India locations: State -> major cities (sample list, can be extended)
INDIA_LOCATIONS = {
    ...
    'Sikkim': ['Gangtok'],
    'Tamil Nadu': ['Chennai', 'Coimbatore', 'Madurai', 'Tiruchirappalli', 'Salem', 'Tirunelveli', 'Erode', 'Vellore', 'Thoothukudi', 'Thanjavur', 'Dindigul', 'Kanchipuram', 'Karur', 'Rajapalayam', 'Nagercoil', 'Kumbakonam', 'Tiruppur', 'Cuddalore', 'Pollachi', 'Kanyakumari'],
...
<body class="bg-gray-50 min-h-screen">
    ...
    <script>
        ...
        // India locations data (state -> cities)
        const INDIA_LOCATIONS = ''' + json.dumps(INDIA_LOCATIONS) + ''';
        ...
        function updateCityOptions(stateSelectId, citySelectId) {
            const stateEl = document.getElementById(stateSelectId);
            const citySelect = document.getElementById(citySelectId);
            if (!stateEl || !citySelect) return;
        ...
        // Handle custom product input and initialize location dropdowns
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize state and city dropdowns for register form
            populateStateDropdown('farmState');
            updateCityOptions('farmState', 'farmLocation');

            const farmState = document.getElementById('farmState');
            if (farmState) {
                farmState.addEventListener('change', function() {
                    updateCityOptions('farmState', 'farmLocation');
                });
            }

            // Initialize state and city dropdowns for update form
            populateStateDropdown('updateState');
            updateCityOptions('updateState', 'updateLocation');

            const updateState = document.getElementById('updateState');
            if (updateState) {
                updateState.addEventListener('change', function() {
                    updateCityOptions('updateState', 'updateLocation');
                });
            }
        });
        ...
        document.getElementById('registerForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const city = document.getElementById('farmLocation').value;
                const fullLocation = state && city ? `${state} - ${city}` : (city || state);
        ...
        document.getElementById('updateForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const updateCity = document.getElementById('updateLocation').value;
                const updateFullLocation = updateState && updateCity ? `${updateState} - ${updateCity}` : (updateCity || updateState);
